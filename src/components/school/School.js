...
import { Layout, Table, Form, Input, Button, Modal, Select } from "antd";
...
class School extends React.Component {
  ...
  getSchools = () => {
    let newArray = [];
    axios
      .get("https://xdemic-api.herokuapp.com/schools")
      .then(res => {
        if (res.data.status) {
  ...
  sendSchool = () => {
    axios
      .post("https://xdemic-api.herokuapp.com/school", this.state)
      .then(res => {
        console.log(res);
        if (res.data.status) {
...
onChange={this.changeHandler} /> */}
          ...
          <Table
            dataSource={this.state.schoolArray}
            columns={columns}
